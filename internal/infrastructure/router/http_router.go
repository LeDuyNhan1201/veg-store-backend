...
func InitHTTPRouter(core *core.Core) *HTTPRouter {
	router := &HTTPRouter{
		...
		ApiPath: core.Config.Server.ApiPrefix + core.Config.Server.ApiVersion,
	}
	...
	if core.Config.Mode != "prod" && core.Config.Mode != "production" {
...
func (router *HTTPRouter) HttpRun() error {
	// Run HTTP Server
	router.Logger.Info(fmt.Sprintf("Starting HTTP server on port %s...", router.Config.Server.Port))
	httpServer := &http.Server{
		Addr:           ":" + router.Config.Server.Port,
...
func initGinEngine(core *core.Core) *gin.Engine {
	...
	logger.UseGinRequestLogging(core.Config.Mode, engine)
	...
	engine.Use(cors.New(cors.Config{
		AllowOrigins:     core.Config.Cors.AllowOrigins,
		AllowMethods:     core.Config.Cors.AllowMethods,
		AllowHeaders:     core.Config.Cors.AllowHeaders,
		AllowCredentials: core.Config.Cors.AllowCredentials,
	}))
...
func (router *HTTPRouter) registerSwaggerUI() {
	docs.SwaggerInfo.Host = router.Config.Swagger.Host
...
func (router *HTTPRouter) AppUse(middlewares ...HttpHandlerFunc) gin.IRoutes {
	return router.Use(adaptHandlers(middlewares...)...)
}

func (router *HTTPRouter) AppGroup(relativePath string, handlers ...HttpHandlerFunc) *gin.RouterGroup {
	return router.Group(relativePath, adaptHandlers(handlers...)...)
}

func (router *HTTPRouter) AppPOST(group *gin.RouterGroup, relativePath string, handlers ...HttpHandlerFunc) gin.IRoutes {
	return group.POST(relativePath, adaptHandlers(handlers...)...)
}

func (router *HTTPRouter) AppGET(group *gin.RouterGroup, relativePath string, handlers ...HttpHandlerFunc) gin.IRoutes {
	return group.GET(relativePath, adaptHandlers(handlers...)...)
}

func (router *HTTPRouter) AppDELETE(group *gin.RouterGroup, relativePath string, handlers ...HttpHandlerFunc) gin.IRoutes {
	return group.DELETE(relativePath, adaptHandlers(handlers...)...)
}

func (router *HTTPRouter) AppPATCH(group *gin.RouterGroup, relativePath string, handlers ...HttpHandlerFunc) gin.IRoutes {
	return group.PATCH(relativePath, adaptHandlers(handlers...)...)
}

func (router *HTTPRouter) AppPUT(group *gin.RouterGroup, relativePath string, handlers ...HttpHandlerFunc) gin.IRoutes {
